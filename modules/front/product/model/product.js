...
function Product(req, res) {
  ...
  this.getProductsList = async (req, res) => {
    ...
    // if (keyword) matchCondition['product_name'] = { $regex: keyword, $options: 'i' };
    ...
    productsList(req, res, matchCondition, sortCondition, limit, skip,finalMatchCondition,searchType).then((data)=>{
      res.send(data);
    })
  }/* end getProductsList*/
  ...
  this.getCategoriesList = (req, res, next) => {
    let collection = db.collection("masters");
    collection.find({ dropdown_type: 'category', status: ACTIVE }, { projection: { name: 1 } }).sort({ 'name': 1 }).toArray(async (err, result) => {
      if (!err && result) {
        return res.send({
          status: API_STATUS_SUCCESS,
          message: res.__("front.user.fetched_category_list_successfully"),
  ...
  this.getSubCategoriesList = (req, res, next) => {
    /* get states list from database*/
    let collection = db.collection("masters");
    const categoryId = req.body.categoryId ? req.body.categoryId : '';
    collection.find({ category_id: ObjectId(categoryId)}, {projection: {name: 1}}).sort({'name': 1}).toArray(async (err, result) => {
      if(!err && result){
        return res.send({
          status: API_STATUS_SUCCESS,
  ...
  this.getDeliveryTypeList = (req, res, next) => {
    let collection = db.collection("masters");
    collection.find({ dropdown_type: 'delivery_types' }, { projection: { name: 1,charge:1 } }).sort({ 'name': 1 }).toArray(async (err, result) => {
      if (!err && result) {
        return res.send({
          status: API_STATUS_SUCCESS,
  ...
  this.getFilterTypeList = (req, res, next) => {
    let collection = db.collection("filter_types");
  ...
  this.getTimesList = (req, res, next) => {
    ...
    collection.aggregate([
      ...
      {$project :	{
        id:1,name:1,modified:1,
        shedules_name : {'$arrayElemAt': ["$categoryDetail.name",0]},
      }}
    ]).toArray((err, result)=>{
      if (!err && result) {
        return res.send({
...
console.log('req hit')
          ...
          pipeline: [
            {
              $match: {
                $expr: {
                  $and: [{ $eq: ['$product_id', '$$ratingProductId'] }],
                },
              },
            },
            {
  ...
  this.postDeleteProduct = async (req, res) => {
    const userId = req.params.userId ? req.params.userId : '';
    const productId = req.params.productId ? req.params.productId : '';
    const products = db.collection('products');
    products.updateOne(
  ...
  // ======filter product
  let productsList = (req, res, matchCondition, sortCondition, limit, skip, finalMatchCondition,searchType) => {
                ...
                $lookup: {
                  ...
                  pipeline: [
                    ...
                    { "$lookup": {
                      ...
                      "pipeline": [
                        { "$match": { "$expr": { "$eq": ["$category_id", "$$subCategoryId"] }}},
                        { $project: { _id: 1, name: 1 } }
                      ],
                      "as": "subCategoryDetail"
                    }}
  ...
  //             // return;
  //           }).catch((escrowError)=>{
  //             console.log("escrowError",escrowError);
              ...
              },
              {
                $project:{
                  ...
                  address: {
                    ...
                    city: { '$arrayElemAt': ['$cityDetail', 0] },
                    pincode: { '$arrayElemAt': ['$shippingAddress.pincode', 0] },
                    street: { '$arrayElemAt': ['$shippingAddress.street', 0] },
                  },
                  phone:'$phone.number'
                }
              }
